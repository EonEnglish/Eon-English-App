...
const ScheduleScreen = () => {
  ...
  const types = [
    { key: "1", value: "Homework Help" },
    { key: "2", value: "Interview" },
  ];
    ...
    fetch(url)
      ...
      .then((data) => {
        ...
        const newCoachDataFiltered = newCoachData.map((item, index) => {
          ...
          // time lapse
          const timeLapseHours =
            parseFloat(durationParts[0]) + parseFloat(timeParts[0]);
          const timeLapseMinutes = `${parseFloat(durationParts[1]) + parseFloat(timeParts[1])}`;
          const timeLapse =
            timeLapseMinutes <= 1
              ? `${timeLapseHours}:${0}${0}`
              : `${timeLapseHours}:${timeLapseMinutes}`;
  ...
  const handleSubmit = () => {
    ...
    const appScriptURL =
      "https://script.google.com/macros/s/AKfycbztbgwE-w-2IkH-zj1jzAYckwnHmcQNMVdtnd9Ds65aoaUXFukq2xkqe8zBtiyshGRu/exec";
    ...
    newErrors.email = [
      ...
      },
      {
        ...
        result: "Email must be in the following format: example@mail.com",
      },
      {
        condition: countOccurrences(email, "@") > 1,
        result: "Email cannot contain more than one @",
      },
      {
        condition: email[email.length - 1] == ".",
        result: "Email cannot end in a .",
      },
      {
        condition:
          email.indexOf(".", email.indexOf("@")) == email.indexOf("@") + 1,
        result: "There must be at least one character between @ and .",
      },
    ];
    newErrors.weChatID = [
    ...
    newErrors.lessons = [
      {
        condition: !userLessonSelect && userTypeSelect == types[0].key,
